  ...
  <div class="news" style="text-align: center">
    ...
    <div class="row" style="display:inline;">
      <div class="col-4 mx-auto" >
        ...
        <ul class="pagination">
          <li class="page-item"><a class="page-link" @click = "getPrevPageOfNewsByWords">Previous</a></li>
          <li class="page-item"><a class="page-link">                      {{ this.pageNum }}</a></li>
          <li class="page-item"><a class="page-link" @click = "getNextPageOfNewsByWords" >Next</a></li>
        </ul>
      </div>
      <div class="col-6" >
...
export default {
  ...
  methods: {
    ...
    getNextPageOfNewsByWords(){
      this.pageNum++
      this.$axios.get(`/api/news/by-word/${this.$route.params.word}?page=${this.pageNum}`).then((response) => {
        this.newsList = response.data;
      });
    },
    getPrevPageOfNewsByWords(){
      if(this.pageNum === 1){
        return
      }
      this.pageNum--
  ...
  mounted() {
    this.$axios.get(`/api/news/by-word/${this.$route.params.word}?page=1`).then((response) => {
      this.newsList = response.data;
    });
  }
}
